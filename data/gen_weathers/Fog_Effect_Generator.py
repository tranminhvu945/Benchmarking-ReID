...
class FogEffectGenerator:
    def __init__(self, device):
        ...
        self._illumination2darkness = {0: 1, 1: 0.85, 2: 0.7, 3: 0.55}
        
        #self._weather2visibility = (500, 2000)
        self._weather2visibility = (300, 1500)
    ...
    def process_image(self, img_path, depth_path, save_path):
        ...
        try:
            foggy = self.genEffect(img_path, depth_path)
            if foggy is not None:
                save_path.parent.mkdir(parents=True, exist_ok=True)
                Image.fromarray(foggy).save(save_path)
                print(f"Saved fog effect to: {save_path}")
                return True
    ...
    def genEffect(self, img_path: str, depth_path: str):
        try:
            ...
            if illumination > 0:
                ...
                I_fog = cv2.GaussianBlur(I_fog, (5, 5), 2)
            ...
            return I_fog.cpu().numpy().astype(np.uint8)
...
def main():
    ...
    foggen = FogEffectGenerator(device)
    ...
    if clear_path.is_file():
        if depth_path.is_file() and depth_path.suffix == ".npy":
            success = foggen.process_image(clear_path, depth_path, 
        ...
        success_total, fail_total = foggen.process_directory(clear_path, depth_path, save_folder)
