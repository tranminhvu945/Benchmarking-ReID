...
class VRU_Rain(BaseImageDataset):
    dataset_dir = 'VRU-Rain'

    def __init__(self, root='', verbose=True, **kwargs):
        super(VRU_Rain, self).__init__()
        ...
        self.query = self._load_data('query_big.txt', self.query_dir, relabel=False)
        self.gallery = self._load_data('gallery_big.txt', self.gallery_dir, relabel=False)

        if verbose:
            print("=> VRU_Rain dataset loaded")
    ...
    def _load_data(self, split_file, dir_path, relabel=False):
        ...
        for line in lines:
            ...
            dataset.append((img_path, pid, 0))  # camid = 0 (m?c d?nh)
