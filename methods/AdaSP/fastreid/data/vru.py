...
class VRU(ImageDataset):
    ...
    def __init__(self, root='', verbose=True, **kwargs):
        ...
        self.query_dir = osp.join(self.dataset_dir, 'images_query_medium')
        self.gallery_dir = osp.join(self.dataset_dir, 'images_gallery_medium')
        self.split_dir = osp.join(self.dataset_dir, 'train_test_split')

        self._check_before_run()
        
        train = self._load_data('train.txt', self.train_dir, relabel=True)
        query = self._load_data('query_medium.txt', self.query_dir, relabel=False)
        gallery = self._load_data('gallery_medium.txt', self.gallery_dir, relabel=False)
