...
class UAV_VeID_Rain(ImageDataset):
    root = "/home/uit2023/LuuTru/Vutm/dataset/"
    dataset_dir = "UAV-VeID-Rain"
    dataset_name = "UAV-VeID-Rain"
    
    def __init__(self, root='', verbose=True, **kwargs):
        self.dataset_dir = osp.join(root, self.dataset_dir)
        self.train_dir = osp.join(self.dataset_dir, 'image_train')
        self.query_dir = osp.join(self.dataset_dir, 'images_query_val')
        self.gallery_dir = osp.join(self.dataset_dir, 'images_gallery_val')
        self.split_dir = osp.join(self.dataset_dir, 'UAV-labels')

        self._check_before_run()
        
        train = self._load_data('train_id_label.txt', self.train_dir)
        query = self._load_data('val_query_label.txt', self.query_dir)
        gallery = self._load_data('val_gallery_label.txt', self.gallery_dir)

        super(UAV_VeID_Rain, self).__init__(train, query, gallery, **kwargs)
